...
static uint8 gps_gngsa_parse (char *line, gps_info_struct *gps)
{
    uint8 state;
    char *buf = line;
    uint8 return_state = 0;
    ...
    if (state != ',')
    {
        gps->pdop=get_double_number(&buf[get_parameter_index(9, buf)]);
        gps->hdop=get_double_number(&buf[get_parameter_index(9, buf)]);
        gps->vdop=get_double_number(&buf[get_parameter_index(9, buf)]);
        return_state = 1;
    }
...
double get_two_points_distance (double latitude1, double longitude1, double latitude2, double longitude2)
{
    ...
    distance = 2 * asin(sqrt(mpow(sin(a / 2), 2) + cos(rad_latitude1) * cos(rad_latitude2) * mpow(sin(b / 2), 2)));   // google maps 里面实现的算法
...
double get_two_points_azimuth (double latitude1, double longitude1, double latitude2, double longitude2)
{
    ...
    double x = msin(longitude2 - longitude1) * mcos(latitude2);
    double y = mcos(latitude1) * msin(latitude2) - msin(latitude1) * mcos(latitude2) * mcos(longitude2 - longitude1);
    double angle = RAD_TO_ANGLE(matan2(x, y));
...
uint8 gps_data_parse (void)
{
    ...
    do
    {
        ...
        if(GPS_STATE_RECEIVED == gps_gga_state)
        {
            ...
            gps.gps_data_used_flag_gngga=1;
        }
        gps_gga_state = GPS_STATE_RECEIVING;

        if(GPS_STATE_RECEIVED == gps_gsa_state)
        {
            gps_gsa_state = GPS_STATE_PARSING;
            strncpy((char *)&check_buffer[2], strchr((const char *)gps_gsa_buffer, '*')+1, 2);
            bbc_xor_origin = (uint8)func_str_to_hex((char *)check_buffer);

            for(bbc_xor_calculation = gps_gsa_buffer[1], data_len = 2; gps_gsa_buffer[data_len] != '*'; data_len ++)
            {
                bbc_xor_calculation ^= gps_gsa_buffer[data_len];
            }
            if(bbc_xor_calculation != bbc_xor_origin)
            {
                // 数据校验失败
            ...
            gps_gngsa_parse((char *)gps_gsa_buffer, &gps_tau1201);
            gps.gps_data_used_flag_gngsa=1;
        }
        gps_gsa_state = GPS_STATE_RECEIVING;
...
void gps_uart_callback (void)
{
    ...
    if(gps_tau1201_state)
    {
        ...
        if('\n' == dat)
        {
            ...
            } else if(0 == strncmp((char *)&temp_gps[3], "GSA", 3))
            {
                // 如果没有在解析数据则更新缓冲区的数据
                if(GPS_STATE_PARSING != gps_gsa_state)
                {
                    gps_gsa_state = GPS_STATE_RECEIVED;
                    temp_length = fifo_used(&gps_tau1201_receiver_fifo);
                    fifo_read_buffer(&gps_tau1201_receiver_fifo, gps_gsa_buffer, &temp_length, FIFO_READ_AND_CLEAN);
                }
            }
...
uint8 gps_init (void)
{
    ...
    const uint8 open_gsa[]      = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x02, 0x01, 0xFC, 0x13};
    ...
    system_delay_ms(1000);                                                           // 等待GPS启动后开始初始化
    ...
    system_delay_ms(50);
    uart_write_buffer(GPS_TAU1201_UART, (uint8 *)open_gsa, sizeof(open_gsa));
    system_delay_ms(50);
    ...
    gps_Administrator_init(&gps);
    gps_read_flash(&gps);
    read_pram_data_flash(&lora);
